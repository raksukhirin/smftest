/* suggest/item-cell.js — Cell-level Item smart-suggest for ExpCode + ExpDesc1
 * across the 3 detail grids (LCL / Shipping / Transport). */
window.SFQ = window.SFQ || {};
window.SFQ.suggestItemCell = (function () {
    'use strict';
    const { $, escapeHtml }            = SFQ.utils;
    const { state, current, setDirty } = SFQ.state;
    const { gridByKind }               = SFQ.factories;
    ...
    function installItemCellSuggest() {
