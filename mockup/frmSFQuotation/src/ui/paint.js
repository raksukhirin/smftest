/* ui/paint.js — All `paint*` functions: re-render UI from current state.
 *
 * Convention: paint* never mutate state — they only push state values into
 * the DOM. The reverse direction (form → state) is `captureForm` in actions
 * and `captureGridRow` in grids/detail. */
window.SFQ = window.SFQ || {};
window.SFQ.paint = (function () {
    'use strict';
    const { $, $$, escapeHtml, formatDate } = SFQ.utils;
    const { state, current, setSuppressDirty, setDirty } = SFQ.state;
    ...
    function paintAll() {
        ...
        SFQ.gridDetail.paintDetail(GRIDS.lcl);
        SFQ.gridDetail.paintDetail(GRIDS.shipping);
        SFQ.gridDetail.paintDetail(GRIDS.transport);
        paintNav();
        paintClassToggle();
        paintTab();
        paintApproveBtn();
    ...
    function paintTab() {
        ...
        $$('[data-panel]').forEach(p => p.classList.toggle('hidden', p.dataset.panel !== state.activeTab));
    }
    ...
    return {
        paintAll, paintHeaderForm, paintBanner, paintNav, paintClassToggle,
        paintTab, paintApproveBtn, applyStatusPill, applyReadOnly, renderSearch,
    };
}());
