 ...
 *
 * - render()       : redraw the cost grid (read-only OR editable mode)
 * - addCostLine()  : append a blank line + redraw
 * - applyToTotal() : sum CTN20..LCL across lines → XCTN20..XLCL header
 * - loadCode()     : refresh service-code combo (placeholder)
 */

import { $, escapeHtml, num, nextPmKey } from './utils.js';
import { state, getCurrentRate, setDirty } from './state.js';
import { showToast } from './ui.js';

/* Re-render the SUB-RATE grid. Mode is determined by whether the
   Rate Detail pane is open (editable) or not (read-only preview). */
...
function wireCostInputs(rate) {
    ...
    tbody.querySelectorAll('[data-cost-field]').forEach(el => {
        el.onchange = () => {
            ...
            // Auto-fill description when service code changes
            if (field === 'EXPCODE') {
                const s = state.data.masters.services.find(x => x.Code === val);
                if (s) rate.costs[idx].EXPDESC1 = s.Desc;
                render(); // re-render to show updated description
            }
            setDirty(true);
        };
    });
...
/** Sum CTN20..LCL across cost lines and copy into header XCTN20..XLCL. */
export function applyToTotal(populateDetailForm) {
    const rate = getCurrentRate();
    if (!rate) return;
...
/** Reload service-code combo (currently just a re-render — placeholder for
 *  future OPItem fetch). */
