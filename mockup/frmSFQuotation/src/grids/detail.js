/* Generic detail-grid renderer driving LCL / Shipping / Transport tabs.
 * Each cfg entry in `GRIDS` (see core/factories.js) routes to the same painter,
 * row factory, and capture function — only `arrayKey`, `tbodyId`, etc. differ. */

import { $, $$, escapeHtml } from '../core/utils.js';
import { current } from '../core/state.js';
import { GRIDS, gridByKind } from '../core/factories.js';

export function paintDetail(cfg) {
    const q = current();
    const tbody = $(`#${cfg.tbodyId}`);
    if (!tbody) return;
    tbody.innerHTML = '';
    // Migration: doc loaded from older seed may not have these arrays.
...
export function detailRowEl(cfg, row, idx) {
    const tr = document.createElement('tr');
    tr.dataset.detid = row.DETID;
    tr.dataset.idx = String(idx);
    tr.dataset.kind = cfg.kind;
    // Common UOM list — Transport uses container sizes too (20'/40').
...
export function recalcDetailTotal(cfg) {
    const q = current();
...
/** Read DOM cell values back into the row object. Called on every input/change. */
export function captureGridRow(tr) {
    const q = current();
    if (!q) return;
...
/** Capture every row across all 4 grids — called before save / print. */
export function captureAllGrids() {
