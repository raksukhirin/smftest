/* Field-level smart-suggest used by the 4 header lookup inputs
 * (CustNo, SaleNo, AppByNo, AgentNo).
 *
 * Mounts an absolutely-positioned dropdown inside `.field-with-lookup`.
 * Search is prefix-first, then substring; capped at SUGGEST_MAX results.
 *
 * Usage:
 *   installSmartSuggest(input, {
 *     source:   () => array,
 *     fields:   ['CustNo', 'CustName'],   // record keys to match against
 *     label:    'customer',                // empty-state text
 *     countLabel: (n) => `${n} customers`, // header chip
 *     render:   (record) => ({ code, name, meta? }),
...
export function installSmartSuggest(codeInput, opts) {
    if (!codeInput) return;
