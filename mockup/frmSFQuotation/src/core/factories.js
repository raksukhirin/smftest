/* core/factories.js — Blank-record factories + grid configurations. */
window.SFQ = window.SFQ || {};
window.SFQ.factories = (function () {
    'use strict';
    const { state } = SFQ.state;

    /** Fresh blank quotation (used by + New / fallback when data fails to load). */
    function blankQuotation(docClass = 'IM') {
        const now = new Date();
    ...
    /** Blank SFQRATE row — VARCHAR rate fields stay strings (so 'FREE'/'20-24' parse). */
    function blankFcl() {
        return {
    ...
    /* Detail-grid configuration (LCL / Shipping / Transport share AFQDETAIL). */
    const GRIDS = {
        lcl: {
    ...
    function gridByKind(kind) { return GRIDS[kind] || null; }

    /** Blank detail row — same shape across LCL/Shipping/Transport (DETID prefix differs). */
    function blankDetail(cfg) {
        return {
    ...
    return { blankQuotation, blankFcl, blankDetail, GRIDS, gridByKind };
}());
